...
// Ejecutar con: node test-register-form.mjs
...
import {
  ...
  formatExpiryDate,
  formatChileanPhone
} from './src/utils/validationUtils.ts';

import { CHILEAN_REGIONS, getCitiesForRegion } from './src/utils/chileData.ts';
...
function testFieldValidation() {
  ...
  // 3. Validación de RUT
  console.log('\n3. RUT:');
  test('RUT válido 12.345.678-5', validateRut('12.345.678-5'));
  test('RUT válido sin puntos 12345678-5', validateRut('12345678-5'));
  test('RUT válido con K 12.345.670-K', validateRut('12.345.670-K'));
  test('RUT válido 7.654.321-6', validateRut('7.654.321-6'));
  test('RUT válido 19.234.567-7', validateRut('19.234.567-7'));
  ...
  test('RUT inválido (muy largo)', !validateRut('123456789-0'));
  test('RUT inválido (sin guion)', !validateRut('123456789'));
  test('RUT inválido (formato incorrecto)', !validateRut('12.345.678'));
  ...
  test('Términos no aceptados', false, false); // Checkbox unchecked, expected false
  test('Privacidad aceptada', true);
  test('Privacidad no aceptada', false, false); // expected false
